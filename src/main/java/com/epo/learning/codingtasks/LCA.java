...
public class LCA {
    ...
    public static TreeNode lca(TreeNode root, int j, int k) {
        Stack<TreeNode> pathToJ = pathToElement(root, j);
        Stack<TreeNode> pathToK = pathToElement(root, k);
        if(pathToJ == null || pathToK == null) {
            return null;
        }

        TreeNode lca = null;
        while (!pathToJ.empty() && ! pathToK.empty()) {
            TreeNode popJ = pathToJ.pop();
            TreeNode popK = pathToK.pop();
            if (popJ == popK) {
                lca = popJ;
            } else {
                break;
    ...
    private static Stack<TreeNode> pathToElement(TreeNode root, int element) {
        if (root == null ) {
            return null;
        }
        ...
        Stack<TreeNode> leftPath = pathToElement(root.left, element);
        if (leftPath != null) {
            leftPath.push(root);
            return leftPath;
        }

        Stack<TreeNode> rightPath = pathToElement(root.right, element);
        if (rightPath != null) {
            rightPath.push(root);
            return rightPath;
        }
